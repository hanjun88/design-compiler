...
/**
 * 验证真实物理资产存在且元数据合法。
 * 读取 step6-a/evidence/ 下的实际执行证据。
 */
export function verifyPhysicalAssets(): AssetGateResult {
  const blockers: string[] = [];
  const evidenceDir = path.join(PROJECT_ROOT, "step6-a", "evidence");

  // 1. 源视频存在
  const sourceVideo = path.join(PROJECT_ROOT, "fixtures", "GOLDEN_CASE_02", "source-video.mp4");
  if (!fs.existsSync(sourceVideo)) {
    blockers.push("source-video.mp4 not found");
  }

  // 2. 媒体元数据证据
  const metadataPath = path.join(evidenceDir, "media-metadata.json");
  ...
  // 3. 关键帧证据
  const keyframesPath = path.join(evidenceDir, "keyframes.json");
  let keyframeCount = 0;
  ...
  // 4. 光流证据
  const motionPath = path.join(evidenceDir, "motion", "motion-summary.json");
  ...
  // 5. 视觉特征证据
  const visualPath = path.join(evidenceDir, "visual-features.json");
...
export interface GoldenCase02ExecutionResult {
...
export async function run(): Promise<RegressionResult> {
  ...
  const pipelineOutput = runner.execute(coreIR, hostCaps, "GOLDEN_CASE_02", { width: 1920, height: 1080, pixelRatio: 1 });
 ...
 */
export async function runDetailed(): Promise<GoldenCase02ExecutionResult & { regression: RegressionResult }> {
  const startTime = Date.now();
  ...
  if (assetGate.passed) {
    ...
    if (normalization.status === "PASS") {
      ...
      const deps = loadPipelineDependencies();
      const runner = new PipelineRunner(deps);
      pipelineOutput = runner.execute(coreIR, hostCaps, "GOLDEN_CASE_02", { width: 1920, height: 1080, pixelRatio: 1 });
