...
const config = JSON.parse(
  fs.readFileSync(path.join(__dirname, "../../config/tier-mapping.json"), "utf8"),
) as TierMappingConfig;
const policy = JSON.parse(
  fs.readFileSync(path.join(__dirname, "../../config/g1-policy.json"), "utf8"),
);
const grammar: GrammarRulePack = {
  packName: "test-grammar",
  version: "1.0.0",
  description: "Step 4 contract fixture",
  rules: [],
};
...
function makeRawIR(overrides: { focalConfidence?: number } = {}): RawDesignIR {
  const raw: RawDesignIR = {
  ...
  if (overrides.focalConfidence !== undefined) raw.composition.focalPoint.confidence = overrides.focalConfidence;
  return raw;
}
...
describe("Pipeline Runner — Step 4 contract", () => {
  test("TC-PR-01: full chain produces SUCCESS, four-hash chain and complete timing", () => {
  ...
  test("TC-PR-02: G1 low-confidence required parameter halts before Patch Engine", () => {
    const result = runner.execute(makeRawIR({ focalConfidence: 0.52 }), caps(), "TC-PR-02");
  ...
  test("TC-PR-03: missing required WebGL2 capability halts at G3", () => {
    ...
    expect(result.evaluation.metrics).toBeUndefined();
    expect(result.evaluation.gates).toBeUndefined();
    expect(result.evaluation.provenance.hashChain).toEqual({
      inputHash: "sha256:0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef",
    });
  });
  ...
  test("TC-PR-04: assembled hash chain is compatible with G2 semantic gate", () => {
    ...
    const evaluation = {
      ...
      metrics: {
        composition: metric,
        color: { composite: metric, palette: metric, dominantArea: metric, temperature: metric, contrast: metric },
        depth: metric,
        material: metric,
        focalPointDisplacement: { ...metric, displacementDistance: 0 },
      },
      gates: {
...
// Keep schema tooling exercised at this boundary without coupling the runner to AJV.
const executionPlanSchema = JSON.parse(
  fs.readFileSync(path.join(__dirname, "../../schemas/execution-plan.schema.json"), "utf8"),
);
const ajv = new Ajv2020({ allErrors: true, strict: false });
const validatePlan = ajv.compile(executionPlanSchema);

test("Step 4 output plan remains execution-plan schema compatible", () => {
  const result = runner.execute(makeRawIR(), caps(), "TC-PR-SCHEMA");
  expect(result.status).toBe("SUCCESS");
  if (result.status !== "SUCCESS") return;
