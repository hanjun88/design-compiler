...
对于给定的编码值 e，原始 NDC n 的范围为：

```
e - 0.5 ≤ (n + 1) × 127.5 + 0.5 < e + 0.5
e - 1 ≤ (n + 1) × 127.5 < e
(e - 1) / 127.5 - 1 ≤ n < e / 127.5 - 1
n' - 1/127.5 ≤ n < n'
```

其中 n' = e/127.5 - 1 为解码值。

**误差**：|n' - n| ∈ [0, 1/127.5] = [0, 2/255]

等等——这给出的最坏情况是 2/255。但让我用具体值验证：

- n = 0：e = floor(127.5 + 0.5) = floor(128) = 128，n' = 128/127.5 - 1 = 0.00392，误差 = 0.00392 = **1/255**
- n → 0.00784（e=128 的上界）：n' = 0.00392，误差 → 0.00392 = **1/255**
- n = -0.00392（e=127 的下界）：e = floor(127.0 + 0.5) = 127，n' = 127/127.5 - 1 = -0.00392，误差 = 0

**精确结论**：对于每个编码桶 [n' - 1/255, n']，解码值 n' 恰好是桶的上界，因此最坏情况量化误差为 **1/255 ≈ 0.00392**（不是 0.5/255，也不是 2/255）。
